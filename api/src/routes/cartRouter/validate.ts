...
export const validate = new (class Validate {
  create() {
    return [
      body("userId").custom(async (userId: number) => {
  ...
  AddToCartItem() {
    return [
      query("tokenCart").custom(async (tokenCart: string) => {
        if (!tokenCart) {
          return true;
        } else {
      ...
      cookie("cart").custom(async (cookie: string) => {
        if (!cookie) {
          return true;
        } else {
  ...
  GetAllCart() {
    return [
      query("userId").custom(async (userId: string) => {
        const id = Number(userId);
        if (Number.isInteger(id) === false) {
          return true;
        } else {
